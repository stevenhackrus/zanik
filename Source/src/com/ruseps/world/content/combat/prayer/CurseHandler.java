package com.ruseps.world.content.combat.prayer;
...
public class CurseHandler {
	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			@Override
			public void execute() {
				if((player.getRights() == PlayerRights.GOLD_MEMBER || player.getRights() == PlayerRights.PLATINUM_MEMBER || player.getRights() == PlayerRights.DIAMOND_MEMBER) && player.getLocation() != Location.WILDERNESS)
					return;
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					for (CurseData curse : CurseData.values()) {
						if (player.getCurseActive()[curse.ordinal()]) {
				...
				if(player.getLocation() != Location.WILDERNESS) {
					if(player.getRights() == PlayerRights.GOLD_MEMBER || player.getRights() == PlayerRights.PLATINUM_MEMBER || player.getRights() == PlayerRights.DIAMOND_MEMBER) {
						drain = 0;
					}
				}
				if (drain <= 0) {
